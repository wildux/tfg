...
for z in range(len(imgs1)):
	...
	for i in range(len(algs1)):
		...
		#cv2.imwrite("resultats/2.1/matches_"+algsName[i]+"_"+str(z)+".png", img3)

		x, y, img3 = vc.homography(imgROI, imgRobot, desROI, desRobot, kp1, kp2, matches)	# Find matching point
		#cv2.imwrite("resultats/2.1/homography_"+algsName[i]+"_"+str(z)+".png", img3)

		#print(name, np.mean(times), "Good matches: "+str(len(good_matches)))
